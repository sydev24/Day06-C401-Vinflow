...
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Thành viên 1 – Data owner                | Research / evidence + chuẩn bị data text từ slide 6 ngày đầu, chunking, embedding, vector DB, retriever. | `data/raw_text/`, `data/processed/chunks.jsonl`, `data/processed/manifest.json`, `data/vectorstore/chroma/`, `src/retriever.py`, screenshot inspect index. |
| Thành viên 2 – SPEC owner                | SPEC + RAG logic + prompt rule, đảm bảo output đúng Answer / Source / Next Action.                       | `docs/thin_spec.md`, `src/prompt.py`, `src/rag_chain.py`, `.env.example`.                                                                                  |
| Thành viên 3 – Prototype owner           | Prototype UI Streamlit, chat box, loading, hiển thị answer/source/next action, sidebar câu hỏi demo.     | `app.py`, screenshot UI, demo chạy được bằng `streamlit run app.py`.                                                                                       |
| Thành viên 4 – Test / failure path owner | Test happy path, low-confidence path, failure path, correction path.                                     | `docs/test_cases.md`, `docs/demo_questions.md`, screenshot kết quả test, bảng pass/fail.                                                                   |
| Thành viên 5 – Demo script / repo owner  | README, demo script, product decision, chuẩn bị repo để nộp và thuyết trình 3–5 phút.                    | `README.md`, `docs/demo_script.md`, `docs/product_decision.md`, video/screenshot backup nếu demo lỗi.                                                      |
